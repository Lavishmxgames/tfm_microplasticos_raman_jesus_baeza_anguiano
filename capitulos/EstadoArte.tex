...
La comparación entre un espectro medido y una biblioteca de referencia requiere una métrica de similitud (o distancia, transformada a similitud) que cuantifique el parecido entre ambos vectores de intensidad. En la práctica de la espectroscopía analítica y del procesamiento de señal se emplean, entre otras, la correlación de Pearson y su variante no paramétrica de Spearman, la similitud coseno y su transformación angular (\emph{Spectral Angle Mapper}, SAM), el \emph{Hit Quality Index} (HQI, equivalente al cuadrado de la similitud coseno y de uso extendido en librerías comerciales de espectroscopía), y las distancias $L_1$ (Manhattan) y $L_2$ (euclídea) sobre vectores normalizados. No hay consenso sobre qué métrica es preferible: su comportamiento relativo depende del tipo de degradación de la señal. Falta, por tanto, una comparación empírica que lo mida.

\subsection{Tecnologías de procesamiento a gran escala}

El paradigma Big Data ofrece mecanismos probados para el procesamiento distribuido de grandes volúmenes de datos, cuyo origen se remonta al modelo \textit{MapReduce} \citep{dean2004mapreduce}, que popularizó la descomposición de un cómputo masivo en tareas independientes distribuidas sobre un clúster de nodos sin memoria compartida. Apache Spark \citep{zaharia2010spark, zaharia2016spark} extendió ese paradigma introduciendo el concepto de \textit{Resilient Distributed Dataset} (RDD) y, posteriormente, las abstracciones DataFrame y Dataset \citep{armbrust2015sparksql}, con las que las operaciones en memoria son mucho más rápidas que los motores basados en disco para cargas iterativas. Su API en Python (PySpark) paraleliza operaciones vectorizadas de forma directa. Este mismo paradigma de cómputo distribuido por clúster se ha aplicado con éxito a otros dominios científicos con una estructura de datos análoga a la de este TFM: por ejemplo, \citet{freeman2014mapping} emplean cómputo distribuido para mapear a escala la actividad cerebral del pez cebra a partir de volúmenes masivos de imágenes, con una carga descomponible en bloques independientes de forma directa. El problema abordado en este TFM encaja en ese mismo patrón, ya que la carga computacional proviene de una combinatoria de comparaciones (polímero~$\times$~nivel de calidad~$\times$~métrica~$\times$~espectro de referencia) fácilmente paralelizable por bloques independientes.
