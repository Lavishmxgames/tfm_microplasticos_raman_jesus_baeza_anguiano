...
Este capítulo recoge el desarrollo técnico del proyecto: la planificación real, el \textit{pipeline} implementado y las decisiones que lo sustentan, los recursos y el presupuesto, la viabilidad, la sostenibilidad y los resultados obtenidos.
...
Las cuatro fases se han completado. Las Fases~1 y~2 concluyeron conforme a lo planificado, y la Fase~3 se ejecutó íntegramente, incluida la ejecución del experimento con datos reales: la base de datos de referencia \emph{Hagelskjær Microplastic Solution Library~1.1} se localizó de acceso público (Zenodo, DOI 10.5281/zenodo.17710809, licencia CC-BY-4.0) y se ejecutó la matriz experimental completa (\texttt{scripts/run\_experiment.py --database Databases/Hagelskjaer --full --degradation}), validada numéricamente frente al protocolo de referencia del director (1885 comparaciones alineadas, idéntico al criterio de validación acordado). La Fase~4 cerró la redacción de los capítulos de resultados, discusión y conclusiones a partir de estos datos reales. La Figura~\ref{fig:gantt} muestra el cronograma del proyecto.
...
\end{tikzpicture}
\caption{Cronograma del proyecto con sus hitos de entrega. La Fase~3, incluida la ejecución del experimento con datos reales, se completó antes de la entrega intermedia.}
\label{fig:gantt}
\end{figure}
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}p{0.22\textwidth}p{0.22\textwidth}p{0.46\textwidth}@{}}
    ...
    \addlinespace
    Empaquetado como librería instalable (\texttt{pyproject.toml}) + CI (GitHub Actions) & Scripts sueltos sin empaquetar & Reproducibilidad, facilita los 97 tests automatizados y evita regresiones al iterar el \textit{pipeline}. \\
    \bottomrule
  \end{tabular}
...
\begin{itemize}
    ...
    \item \textbf{Viabilidad temporal:} el riesgo más relevante identificado durante el proyecto era la dependencia de la base de datos de referencia. Se mitigó localizándola de acceso público y ejecutando el experimento completo antes de la entrega intermedia (7~sep), lo que dejó el margen restante del calendario para consolidar Resultados, Discusión y Conclusiones sobre las cifras ya obtenidas.
...
Se han alcanzado los siguientes hitos técnicos, cada uno vinculado con su objetivo específico. La Fase~3 se completó íntegramente, por lo que esta sección recoge resultados obtenidos con la base de datos de referencia real, no sintética:
